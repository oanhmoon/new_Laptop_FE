...
//   // 🟢 Gán userId từ URL khi chuyển từ trang UserManagement
//   useEffect(() => {
//     if (userIdFromParam) {
...
//         .then(() => {
//           // Nếu có tin nhắn, messagesFromStore sẽ tự cập nhật
//         })
//         .catch(() => {
//           // Nếu chưa có cuộc trò chuyện, tạo mảng trống để hiển thị
//           setMessages([]);
//         });
//     }
...
//     // Hiển thị ngay tin nhắn trong UI
//     setMessages((prev) => [...prev, msg]);
//     setMessageInput("");
//     scrollToBottom();
//   };
...
//                 Chưa có tin nhắn nào — hãy bắt đầu cuộc trò chuyện mới 👋
//               </div>
//             ) : (
//               messages.map((m, i) => (
//                 <div
...
const ChatManager = () => {
  ...
  const sendMessage = () => {
    ...
    // ⚠️ Không setMessages ở đây để tránh bị trùng tin nhắn
    setMessageInput("");
    scrollToBottom();
  };
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // ⚠️ Không setMessages ở đây để tránh duplicate
      scrollToBottom();
    } catch (err) {
      console.error("Upload error:", err);
      antdMessage.error("Tải tệp thất bại");
